...
import { createAppleNotesServer, parseAppleNotesMode, SERVER_INSTRUCTIONS, SERVER_VERSION } from "../src/server.js";
import { runJxaProcess, type JxaProcessExecutor, type JxaRunner } from "../src/jxa.js";
import { SafeToolError } from "../src/errors.js";
import { parseAllowRawHtml, parseAllowSharedWrites } from "../src/write-policy.js";
...
function startServer(mode: string | undefined, extraEnv: NodeJS.ProcessEnv = {}): ServerProcess {
  ...
  const exited = new Promise<{
    ...
    proc.once("exit", (code, signal) => {
      for (const waiter of pending.values()) {
        waiter.reject(new Error(`Server exited before responding (code ${code}, signal ${signal})`));
      }
      pending.clear();
      resolve({ code, signal });
    });
...
test("default startup advertises only the four read tools", async () => {
  assert.deepEqual(await advertisedTools(undefined), ["list_folders", "list_notes", "search_notes", "get_note"]);
});

test("explicit read-only startup advertises only the four read tools", async () => {
  assert.deepEqual(await advertisedTools("read-only"), ["list_folders", "list_notes", "search_notes", "get_note"]);
});
...
test("mutation schemas require stable full folder/note ids and expose no name selector", async () => {
  ...
  try {
    ...
    assert.equal(
      tools.some((tool) => tool.name === "delete_note"),
      false,
    );
  ...
  await withInjectedServer(
    ...
    async (client) => {
      const calls = [
        {
          name: "create_note",
          arguments: { title: "x", body: "", folder: "Work" },
        },
  ...
  await withInjectedServer(
    ...
    async (client) => {
      for (const call of [
        {
          name: "update_note",
          arguments: { id: "x-coredata://A/ICNote/p1", body: "x" },
        },
        {
          name: "move_note",
          arguments: {
        ...
        {
          name: "trash_note",
          arguments: { id: "x-coredata://A/ICNote/p1", confirm: true },
        },
      ]) {
        const result = await client.callTool(call);
        assert.equal(result.isError, true);
...
test("initialize returns the derived version and security boundary instructions", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /never invokes Notes' delete command.*permanent-delete operation/i,
    );
...
test("process-boundary write failures preserve unknown outcomes and dry-run certainty", async () => {
  ...
  const cases: {
    ...
    {
      label: "timeout",
      executor: (_file, _args, _options, callback) =>
        callback(Object.assign(new Error(secret), { code: "ETIMEDOUT", killed: true }), secret, secret),
      call: {
        name: "create_note",
        arguments: {
    ...
    },
    {
      ...
      executor: (_file, _args, _options, callback) => callback(new Error(secret), secret, secret),
      call: {
        name: "trash_note",
        arguments: {
          id: noteId,
...
test("safe JXA domain errors survive write handling unchanged", async () => {
  const runner: JxaRunner = async <T>() => {
    throw new SafeToolError("CONFLICT", "The note changed after it was read. Re-read it before retrying.");
  };
  await withInjectedServer(
    "read-write",
    runner,
...
test("invalid shared-write configuration exits before startup", async () => {
  const server = startServer("read-write", {
    APPLE_NOTES_ALLOW_SHARED_WRITES: "TRUE",
  });
  const { code } = await server.exited;
  assert.notEqual(code, 0);
...
test("invalid case-mismatched mode exits before startup with a clear error", async () => {
  ...
  const { code } = await server.exited;
  assert.notEqual(code, 0);
  assert.match(server.stderr(), /Invalid APPLE_NOTES_MODE "READ-WRITE".*Expected exactly "read-only" or "read-write"/);
  assert.doesNotMatch(server.stderr(), /server running on stdio/);
});
    ...
    folders: [
      {
        id: trashA,
        name: "Bin A",
        account: { id: accountA, name: "A" },
        count: 0,
      },
      {
        id: trashB,
        name: "Bin B",
        account: { id: accountB, name: "B" },
        count: 0,
      },
    ],
  };

...
test("process invocation fixes the executable, argv, bounds, and minimal environment", async () => {
  ...
  const result = await runJxaProcess<{ ok: boolean }>(
    executor,
    "SCRIPT_SECRET",
    ["ARGV_SECRET"],
    {
...
test("automation process errors never disclose stderr, script, argv, or bodies", async () => {
  ...
  await assert.rejects(
    runJxaProcess(executor, "SCRIPT_SECRET", ["ARGV_SECRET"]),
    (error: Error) => {
...
test("timeout and output overflow are bounded and sanitized", async () => {
  const timeoutExecutor: JxaProcessExecutor = (_file, _args, options, callback) => {
    ...
    callback(
      processError("PRIVATE TIMEOUT DETAIL", { code: "ETIMEDOUT", killed: true }),
      "PRIVATE STDOUT",
      "PRIVATE STDERR"
    );
  };
  await assert.rejects(
    runJxaProcess(timeoutExecutor, "SCRIPT_SECRET", ["ARGV_SECRET"]),
    (error: Error) => {
      assert.equal(
        error.message,
        "Apple Notes automation exceeded its execution time limit"
      );
  ...
  await assert.rejects(
    runJxaProcess(overflowExecutor, "SCRIPT_SECRET", ["ARGV_SECRET"]),
    (error: Error) => {
...
test("unexpected stdout is replaced while explicit safe domain errors survive", async () => {
  ...
  const safeExecutor: JxaProcessExecutor = (_file, _args, _options, callback) => {
    callback(
      null,
      '{"__appleNotesSafeError":true,"code":"NOTE_NOT_FOUND","message":"Note was not found."}',
      ""
    );
  };

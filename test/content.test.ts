...
import { CONTENT_LIMITS, plainTextToHtml, prepareContent, sanitizeNotesHtml } from "../src/content.js";
...
test("plain content is the default projection and never auto-detects markup", () => {
  ...
  assert.equal(plainTextToHtml("  <div>x</div>"), "<div>  &lt;div&gt;x&lt;/div&gt;</div>");
});
...
test("raw HTML accepts and canonicalizes the small balanced subset", () => {
  ...
  assert.equal(prepareContent("<p>trusted</p>", "html", true).html, "<p>trusted</p>");
  assert.equal(

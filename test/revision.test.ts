...
test("Node and JXA revision encoders are identical and retain milliseconds", () => {
  ...
  assert.equal(
    jxa.revisionToken(id, accountId, folderId, modified),
  ...
  assert.equal(
    jxa.noteRevision(
      ...
      id,
      { account: { id: accountId }, folder: { id: folderId } }
    ),
    revisionToken(id, accountId, folderId, modified)
  );
...
test("get-note observation rejects content-time and location races conservatively", () => {
  ...
  const stable = jxa.consistentReadRevision(
    "note", initial, "2026-01-01T00:00:00.000Z",
    initial, "2026-01-01T00:00:00.000Z", []
  );
  assert.equal(
    stable,
    revisionToken("note", "account", "folder-a", "2026-01-01T00:00:00.000Z")
  );
  assert.throws(
    () => jxa.consistentReadRevision(
      "note", initial, "2026-01-01T00:00:00.000Z",
      initial, "2026-01-01T00:00:01.000Z", []
    ),
...
test("a folder-only move changes revision without a timestamp change", () => {
  ...
  const modified = "2026-08-06T01:02:03.987Z";
  assert.notEqual(
    revisionToken(...common, "folder-a", modified),
    revisionToken(...common, "folder-b", modified)
  );
});

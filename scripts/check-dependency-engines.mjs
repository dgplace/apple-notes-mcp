...
const sdkDirectory = fileURLToPath(new URL("../node_modules/@modelcontextprotocol/sdk/", import.meta.url));
const dependencyEntry = require.resolve("@hono/node-server", {
  paths: [sdkDirectory],
});
const dependency = JSON.parse(await readFile(new URL("../package.json", pathToFileURL(dependencyEntry)), "utf8"));

assert.equal(root.engines?.node, ">=18.14.1", "the package must accurately declare its Node 18 patch-level minimum");
assert.equal(
